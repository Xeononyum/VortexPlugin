...
public class Database {
    ...
    public static Mono<UpdateResult> setBan(BanData data) {
        return Mono.from(bansCollection.replaceOne(getBanFilter(data.uuid, data.ip), data,
    ...
    private static Bson getBanFilter(String uuid, String ip) {
        return or(and(eq("uuid", uuid), eq("server", config.gamemode.name())),
                and(eq("ip", ip), eq("server", config.gamemode.name())));
    }
}
