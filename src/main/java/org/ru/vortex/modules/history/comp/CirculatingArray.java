...
public class CirculatingArray<T> {
    ...
    public boolean add(T element) {
        if (deletePointer >= staticSize - 1)
            deletePointer = 0;

        if (array[deletePointer] != null)
            onRemove.accept(deletePointer);
    ...
    public boolean contains(Function<T, Boolean> elementFinder) {
        return indexOf(elementFinder) == -1 ? false : true;
    }
    ...
    public int indexOf(Function<T, Boolean> elementFinder) {
        for (int i = 0; i < staticSize; i++) {
            ...
            if (array[i] == null)
                return -1;
            if (elementFinder.apply(element).booleanValue())
                return i;
        }
