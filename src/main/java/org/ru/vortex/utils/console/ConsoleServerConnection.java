    ...
    public void run() {
        ...
        try {
            ...
            if (!input.equals(outlinePassword)) {
                ...
                Log.err("[REMOTE_CONSOLE]" + socket.getInetAddress().getHostAddress() + " authorization FAILED");
                this.downService();
            } else {
                Log.info("[REMOTE_CONSOLE]" + socket.getInetAddress().getHostAddress() + " authorization DONE");
            }
            while (true) {
                input = in.readLine();
